...
class RegistrationView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            user_serializer = UserListSerializer(user, context={"request": request}).data
            ...
            email_body = "Hi " + user_serializer["name"] + " Use the link below to verify your email \n" + absurl

            data = {"email_subject": email_subject, "email_to": (user_serializer["email"],), "body": email_body}

            sendEmailVerification.delay(data)

            return Response({"user": user_serializer, "token": auth_data}, status=status.HTTP_201_CREATED)
...
class EmailVerifyView(APIView):
    def get(self, request):
        ...
        try:
            ...
            if not user:
                return Response({"error": "Not user"}, status=status.HTTP_400_BAD_REQUEST)
            ...
            return Response({"error": "Activations error"}, status=status.HTTP_400_BAD_REQUEST)
